export default async function handler(req, res) {
  ...
  const prompts = {
    ...
    applewatch: `Analiza esta captura de pantalla del resumen de un entrenamiento de Apple Watch (app Fitness/Entrenamiento). Extrae, si están visibles: duración total en minutos (number), calorías activas quemadas (number), y frecuencia cardiaca promedio en latidos por minuto (number). Responde SOLO con un JSON con exactamente estas claves: duracion, calorias, fcPromedio. Usa null en cualquier valor que no puedas leer con certeza. No inventes números.`,
    ...
    nutrients: `Analiza esta captura de pantalla o foto de una etiqueta nutricional o app de nutrición. Extrae, si están visibles: fibra en gramos (number), azúcares en gramos (number), sodio en miligramos (number), colesterol en miligramos (number), potasio en miligramos (number), calcio en miligramos (number), hierro en miligramos (number), vitamina C en miligramos (number), vitamina D en microgramos (number), y la fecha en formato YYYY-MM-DD si aparece (string o null). Responde SOLO con un JSON con exactamente estas claves: fiber, sugar, sodium, cholesterol, potassium, calcium, iron, vitaminC, vitaminD, date. Usa null en cualquier valor que no puedas leer con certeza. No inventes números.`,
    hormones: `Analiza esta foto o captura de un resultado de laboratorio de hormonas. Extrae, si están visibles: estradiol en pg/mL (number), FSH en mIU/mL (number), LH en mIU/mL (number), progesterona en ng/mL (number), testosterona en ng/dL (number), TSH en mIU/L (number), T4 libre en ng/dL (number), T3 libre en pg/mL (number), y la fecha del estudio en formato YYYY-MM-DD si aparece (string o null). Responde SOLO con un JSON con exactamente estas claves: estradiol, fsh, lh, progesterona, testosterona, tsh, t4Libre, t3Libre, date. Usa null en cualquier valor que no puedas leer con certeza. No inventes números.`,
  };
  ...
  try {
    ...
    if (!text) {
      return res.status(502).json({ ok: false, error: 'No se pudo leer el archivo' });
    }
    ...
    return res.status(500).json({ ok: false, error: 'Error al procesar el archivo' });
  }
}
